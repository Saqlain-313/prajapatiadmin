        ...
        <div className="relative z-10 p-6">
          ...
          <div className="flex gap-3 justify-end">
            ...
            <button
              ...
              className={isApprove ? approveButtonClass : rejectButtonClass}
            >
              {isApprove ? (
                <>
...
/* --------------------------------------------------------
   MAIN COMPONENT — Admin Deposits (only show 'rejected' deposits)
-------------------------------------------------------- */
const AdminRejectedDeposits = () => {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-black via-[#0A0C0F] to-[#030405] p-4 md:p-6 lg:p-8">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-white/40 text-sm mt-0.5 flex items-center gap-2">
                <span>{rejectedDeposits.length} rejected</span>
              </p>
            </div>
          </div>
        </div>
      ...
      {/* Deposits Table */}
      {!loading && rejectedDeposits.length > 0 && (
                    ...
                    <td className="p-4">
                      <div className="relative">
                        ...
                        <input
                          ...
                          // For rejected, always disabled (cannot edit)
                          disabled={true}
                        />
                      </div>
                    </td>
      ...
      {!loading && rejectedDeposits.length === 0 && (
        <div className={`${gradientCardClass} p-12 text-center`}>
          ...
          <p className="text-white/50 text-lg font-medium">No rejected deposit requests found</p>
          <p className="text-white/30 text-sm mt-1">Rejected deposits will appear here</p>
        </div>
      )}

      {/* Confirmation Modal (kept for code parity; will not show as there are no approve/reject actions in this view) */}
      <ConfirmationModal
        isOpen={confirmation.isOpen}
        onClose={() => setConfirmation({ ...confirmation, isOpen: false })}
        onConfirm={handleConfirmAction}
        type={confirmation.type}
        title={confirmation.type === "approve" ? "Approve Deposit" : "Reject Deposit"}
        message={
          confirmation.type === "approve"
            ? "Are you sure you want to approve this deposit request? The user's wallet will be credited."
            : "Are you sure you want to reject this deposit request? The user will be notified."
        }
      />
